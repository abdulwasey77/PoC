...
#define MQ135Am    36  //a
#define LDRpin     35  //a
#define waterPin   33  //a

#define RLAm 22  //The value of resistor RL is 20K
#define mAm -0.404215 //Enter calculated Slope 
#define bAm 0.864883 //Enter calculated intercept
#define RoAm 5 //Enter found Ro value
...
#define RoCO2 35 //Enter found Ro value
...
//--------------------long------------------//
unsigned long displayTimer = millis(), changeDisplay = millis(),simST=millis(), timeOTP;
unsigned long NBtime = 0, BBtime = 0;
unsigned long min5 = millis(), sec10 = millis();
unsigned long postmillis,dataGetTime;
...
char fail = 1;
char displayOTP, simReady = 0,APFlag=0;
char postAPI = 1, Action = 0, Resp = 0, postapiFlag = 0, actionFlag = 0, readresFlag = 0, readComp = 1;


//--------------------string--------------//
String battPrcnt, simPrcnt, wifiPrcnt, hum, temp, HI, CO2, CO, Ammo,O2,ldr;
String Humidity, Temperature, heatIndex, CO2gas, COgas, Ammonia,O2gas;
...
MHZ19 *mhz19_uart = new MHZ19(CO2RX,CO2TX);   //CO2 Sensor MHZ19 Usart Setting
...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(nextButton), nxtBtn, CHANGE);  //Interrrupt on button
...
void loop()
{
  ...
  if (simFlag && !simReady && millis()-simST>60000){
    simST=millis();
    SIMsignal();
  }
  getToken();
...
void postData() {
  if (simFlag && !getFlag) {
    ...
    PostStr = "AT+HTTPPARA=\"URL\",\"backoffice.poultryapp.com/backup_api/automate/dataStoring?accessToken=" + accessToken + "&temp="+Temperature+"&humi="+Humidity+"&heat="+heatIndex+"&co2g="+CO2gas+"&coga="+COgas+"&ammo="+Ammonia+"&airf=&ldrl="+ldr+"&deviceDateTime="+deviceDateTime+"&mode="+sendmode+"\"";
    simReady = 1;
    postFlag=1;
    ...
    PostStr = "https://backoffice.poultryapp.com/backup_api/automate/dataStoring?accessToken=" + accessToken + "&temp="+Temperature+"&humi="+Humidity+"&heat="+heatIndex+"&co2g="+CO2gas+"&coga="+COgas+"&ammo="+Ammonia+"&airf=&ldrl="+ldr+"&deviceDateTime="+deviceDateTime+"&mode="+sendmode;
    postWifiAPI();
...
void getData() {
  if (simFlag && !postFlag) {
    ...
    PostStr = "";
    PostStr = "https://backoffice.poultryapp.com/backup_api/automate/dataForApplications?accessToken=" + accessToken + "&fans=true&lights=false&foggers=true&splinkers=false&deviceDateTime="+deviceDateTime+"&mode="+sendmode+"&dateTime=2020-11-21T15:52:28.510Z";
    postWifiAPI();
    if (httpCode == 400 || httpCode == 401 || httpCode == 403) {
      accessFlag = 1;
      refreshFlag = 1;
...
//  Serial.print("sensor_value RAW of MQ135Am: "); Serial.println(analogRead(MQ135Am));
  VRL = ((analogRead(MQ135Am)*3.3) / 4096);
...
//  Serial.print("sensor_value RAW of MQ135Am: "); Serial.println(analogRead(MQ135Am));
  VRL = ((analogRead(MQ135Am) * 3.3) / 4096); 
//  Serial.print("RAW voltage of MQ135Am: "); Serial.println(VRL);
  VRL = (VRL*5)/3.3; //Measure the voltage drop and convert to 0-5V
//  Serial.print("RAW voltage of MQ135Am 5V: "); Serial.println(VRL);
  Rs = ((5 * RLCO2) / VRL) - RLCO2; //Use formula to get Rs value
...
//  Serial.print("ratio: "); Serial.println(ratio);
 ...
 // Serial.print("CO2 ppm: "); Serial.println(ppm);   // set the cursor to column 0, line 1
  CO2="";
  CO2 += String(int(ppm)) + "ppm";
  CO2gas = String(ppm);
}
...
void readMQ7() {
  ...
  float ratio; //Define variable for ratio
  float sensorValue=0;//Read analog values of sensor
  for(int i=0 ; i<10 ;i++){
  sensorValue = sensorValue+analogRead(MQ7CO); //Read analog values of sensor  
  delay(1);
  }
  sensorValue=sensorValue/10;
//  Serial.println();
//  Serial.print("sensor_value RAW of MQ7: "); Serial.println(sensorValue);
  sensor_volt = ((sensorValue *3.3) / 4096); //Convert analog values to voltage
//  Serial.print("sensor_volt 3.3: "); Serial.println(sensor_volt);
  sensor_volt=(sensor_volt/3.3)*5;
//  Serial.print("sensor_volt 5: "); Serial.println(sensor_volt);
  RS_gas = ((5.0 * 17) / sensor_volt) - 17; //Get value of RS in a gas
...
void getBatt() {
  ...
  //Serial.print("Battvalue: "); Serial.println(Battvalue);
  Battperct=map(Battvalue,2700,3100,0,100);
  battPrcnt="";
  battPrcnt += String(int(Battperct)) + "%";
  //Serial.print("battPrcnt: "); Serial.println(battPrcnt);
}
...
void STA_config() {
  ...
  for (int tr = 0; tr <= 30; tr++)    //Checking if router has 
  {
