...
#define wateEmt  320
#define ldrVal  2700
/*Time For SIM and WiFi posting 
 * Reducing it too much can effect functionality of APmode
 * Current time defined below is in milisecounds 1sec=1000
 * Time for SIM is set to 20sec which can be reduced to 12sec
 * Time for WiFi is set to 5sec which can be reduced to 3sec*/
#define simTimeSec  20000 
#define wifiTimeSec  5000
...
//--------------------string--------------//
String battPrcnt, simPrcnt, wifiPrcnt, hum, temp, HI, CO2, CO, Ammo, O2, ldr,airP,waterPres;
String Humidity, Temperature, heatIndex, CO2gas, COgas, Ammonia, O2gas,airPres;
...
void postData() {
  if (simFlag) {
    ...
    PostStr = "AT+HTTPPARA=\"URL\",\"backoffice.poultryapp.com/backup_api/automate/dataStoring?accessToken=" + accessToken + "&temp=" + Temperature + "&humi=" + Humidity + "&heat=" + heatIndex + "&co2g=" + CO2gas + "&coga=" + COgas + "&ammo=" + Ammonia + "&airf="+airP+"&ldrl=" + ldr + "&wate=" + waterPres + "&deviceDateTime=" + deviceDateTime + "&mode=" + sendmode + "\"";
    postSim900API();
  ...
  } else if (WiFiConnectFlag)
  {
    ...
    PostStr = "https://backoffice.poultryapp.com/backup_api/automate/dataStoring?accessToken=" + accessToken + "&temp=" + Temperature + "&humi=" + Humidity + "&heat=" + heatIndex + "&co2g=" + CO2gas + "&coga=" + COgas + "&ammo=" + Ammonia + "&airf="+airP+"&ldrl=" + ldr + "&wate=" + waterPres + "&deviceDateTime=" + deviceDateTime + "&mode=" + sendmode;
    postWifiAPI();
...
void readPressure(){
  ...
  airPres=pressure;
  airPres+="Pa"; 
  Serial.print("Air Press: "); Serial.println(airP);
}
